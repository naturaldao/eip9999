...
本文档中描述的智能合约接口集不兼容现有EIP标准。该标准引入了slog、source两重结构模型，灵活地实现了元宇宙公共资源的登记。~~该标准引入了新的转移与授权模型。~~

标准引入了slot使得合约可以纵向扩展，与source形成一对多的关系。

由slotId和sourceId共同定义一个具体的公共资源，标识每个资源的独一无二性。

这样就形成了一个类似菜单的关系，并可构建出一个可供检索的公开透明的元宇宙公共资源库。使得资源的整合与运用更加灵活。
